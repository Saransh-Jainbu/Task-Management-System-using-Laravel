    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('tasks.update', $task) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title" id="title" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition @error('title') border-red-500 @else border-gray-300 @enderror" value="{{ old('title', $task->title) }}">
                @error('title')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">{{ old('description', $task->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-8">
                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="is_completed" value="1" class="form-checkbox h-5 w-5 text-indigo-600 rounded focus:ring-indigo-500 border-gray-300" {{ old('is_completed', $task->is_completed) ? 'checked' : '' }}>
                    <span class="text-gray-700 font-medium">Mark as Completed</span>
                </label>
            </div>
